...
@if ($type == 'default')
    @if ($confirmType == 'disabled')
        <!-- Modal toggle -->
        <div class="flex justify-center">
            <button disabled id="{{ $name }}Button" data-modal-target="{{ $name }}Modal"
                data-modal-toggle="{{ $name }}Modal"
                class="block w-24 text-white bg-primary-700 cursor-not-allowed hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                type="button">
                {{ $btnLabel }}
            </button>
        </div>
    @elseif ($confirmType == 'normal')
        <!-- Modal toggle -->
        <div class="flex justify-center">
            <button id="{{ $name }}Button" data-modal-target="{{ $name }}Modal"
                data-modal-toggle="{{ $name }}Modal"
                class="block w-24 text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                type="button">
                {{ $btnLabel }}
            </button>
        </div>
                ...
                <div class="relative w-full h-full max-w-md p-4 md:h-auto">
                    ...
                    <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div class="flex items-center justify-center space-x-4">
                            ...
                            <button form="{{$name}}-submitForm" {{ $confirmType }} type="submit"
                            ...
                            <form hidden id="{{$name}}-submitForm" class="hidden" action="{{ $formAction }}" method="POST">
                                @csrf
                                @method('POST')
                                <input hidden class="hidden" id="{{ $inputId1 }}" name="{{ $inputId1 }}" type="number"
                                    value="{{ $data1 }}" />
                                <input hidden class="hidden" id="{{ $inputId2 }}" name="{{ $inputId2 }}" type="number"
                                    value="{{ $data2 }}" />
                            </form>
                        </div>
                    </div>
...
@elseif ($type == 'warning')
    @if ($confirmType == 'disabled')
        ...
        <div class="flex justify-center">
            <button disabled id="{{ $name }}Button" data-modal-target="{{ $name }}Modal"
                data-modal-toggle="{{ $name }}Modal"
                class="block w-24 text-white cursor-not-allowed bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
                type="button">
                {{ $btnLabel }}
            </button>
        </div>
